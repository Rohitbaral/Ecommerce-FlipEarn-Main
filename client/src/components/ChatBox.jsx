...
if(!isOpen || !listing) return null
  ...
  return (
    ...
    bg-opacity-50 z-100 flex items-center justify-center sm:p-4'>
      ...
      max-w-2xl h-screen sm:h-[500px] flex flex-col'>
       {/* Header */}
       ...
       p-4 sm:rounded-t-lg flex items-center justify-between'>
           <div className='flex-1 min-w-0'>
              ...
              <p className='text-sm text-indigo-100 truncate'>{user.id == listing?.ownerId ? `Chatting with buyer (${chat?.chatUser?.name 
                || 'Loading...'})` : `Chatting with seller (${chat?.ownerUser?.name || 'Loading...'})`}</p>
           </div>
            ...
            <div className='flex items-center justify-center h-full'>
                ...
                messages.map((message)=>(
                    <div key={message.id} className={`flex ${message.sender_id === 
                       ...
                       <div className={`max-w-[70%] rounded-lg p-3 pb-1 ${message.sender_id ===
                        user.id ? "bg-indigo-600 text-white" : "bg-white border border-gray-200 text-gray-800"}`}>
                        <p className='tex-sm break-words whitespace-pre-wrap'>
                            {message.message}</p>
                        <p className={`text-[10px] mt-1 flex items-center gap-1 ${message.sender_id ===
    ...
    (
        <form onSubmit={handleSendMessage} className='p-4 bg-white border-t border-gray-200 rounded-b-lg'>
            <div className='flex items-end space-x-2'>
                ...
                resize-none border border-gray-300 rounded-lg px-4 py-2 focus:outline-indigo-500
                 max-h-32' rows={1}/>

                 <button disabled={!newMessage.trim() || isSending} type='submit' className='bg-indigo-600 
                 hover:bg-indigo-700 text-white p-2.5 rounded-lg
