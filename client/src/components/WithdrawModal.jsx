...
const WithdrawModal = ({onClose}) => {
  ...
  const handleSubmission = async (e) =>{
    e.preventDefault();
     try {
      ...
      for (const field of account) {
        ...
        if (field.name.toLowerCase().includes('email') || field.type === 'email') {
          ...
          if (!emailRegex.test(field.value)) {
            return toast.error(`Please enter a valid email address for ${field.name}`);
          }
        }
      }
      ...
      const {data} = await api.post('/api/listing/withdraw', {account, 
  ...
  return (
    <div className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50
    z-100 flex items-center justify-center sm:p-4'>
      ...
      sm:h-auto flex flex-col'>
        ...
        p-4 sm:rounde-t-lg flex items-center justify-between'>
        ...
        gap-4 p-4 overflow-y-scroll'>
            <div className='grid grid-cols-[2fr_3fr_1fr] items-center gap-2'>
               Amount <input onChange={(e)=> setAmount(e.target.value)}
                value={amount} type='number' className='w-full px-2 py-1.5 text-sm
            ...
            {account.map((field, index)=>(
                ...
                items-center gap-2'>
                   ...
                   <input type={field.type} value={field.value} onChange={(e) =>
                    setAccount((prev)=>prev.map((c, i)=>(i === index ? {...c, value: e.target.value} 
                    : c)))} className='w-full px-2 py-1.5 text-sm border border-gray-300 rounded
                     outline-indigo-400'/>
                </div>
            ))}
