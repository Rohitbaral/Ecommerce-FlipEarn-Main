...
useEffect(() => {
  const fetchListing = async () => {
    try {
      ...
      const reduxListing = listings.find((listing) => listing.id === listingId);
      if (reduxListing) setListing(reduxListing);
    }
  };

  fetchListing();
}, [listingId, listings, getToken]);
  ...
  return listing ? (
   ...
   <div className='flex items-start max-md:flex-col gap-10'>
            ...
            <div>
              ...
              <div className='flex gap-2 mt-2'>
                {listing.verified && (
                 ...
                 {listing.monetized && (
                  <span className='flex items-center text-xs bg-yellow-50 text-yellow-700 
                  px-2 py-1 rounded-md border border-yellow-100'>
                    <div className='flex items-center justify-center w-4 h-4 bg-yellow-100 rounded-full border border-yellow-200 mr-1.5'>
                        <DollarSign className='text-yellow-600 w-2.5 h-2.5 stroke-[3px]' />
                    </div>
                    Monetized
                  </span>
                )}
          ...
          overflow-hidden'>
           ...
           {/* Slider container */}
             <div className='relative w-full aspect-video overflow-hidden'>
                 ...
                 gap-2'>
                  {images.map((_, index)=>(
                    <button onClick={()=>setCurrent(index)} key={index} className={`w-2.5 h-2.5 rounded-full ${current === index ? "bg-indigo-600"
                      : "bg-gray-300"}`}/>
                  ))}
                 </div>
             </div>
        ...
        <div className='bg-white rounded-xl border border-gray-200 mb-5'>
           ...
           <div className='grid grid-cols-2 md:grid-cols-4 gap-4 p-4 text-center'>
             ...
             <div>
              ...
              <p className='font-semibold text-gray-800'>
                {listing.monthly_views?.toLocaleString('en-IN')}
              </p>
    ...
    border-gray-200 p-5 max-md:mb-10'>
      ...
      <button onClick={loadChatbox} className='w-full bg-indigo-600 text-white py-2
      rounded-lg hover:bg-indigo-700 transition text-sm font-medium flex items-center
      ...
      <button  
        ...
        className={`w-full mt-2 ${isProcessing ? 'bg-purple-400' : 'bg-purple-600 hover:bg-purple-700'} text-white py-2 rounded-lg transition text-sm font-medium flex items-center justify-center gap-2`}
      >
