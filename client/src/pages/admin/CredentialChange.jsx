...
const CredentialChange = () => {
    ...
    const fetchAllUnchangedListings = async () => {
        try {
            ...
            const {data} = await api.get('/api/admin/unchanged-listings', {headers: {
                Authorization: `Bearer ${token}`
            }})
            setListings(data.listings || []) // error part fixed
            setLoading(false)
        } catch (error) {
    ...
    return loading ? (
        ...
        <div className='h-full'>
            {listings?.length === 0 ? ( // error part fixed
                ...
                <>
                    ...
                    <div className='mt-10 overflow-x-auto bg-white border border-gray-200 w-full max-w-5xl rounded-xl'>
                        <table className='w-full text-sm text-left  text-gray-700  '>
                            ...
                            <tbody>
                                {listings?.map((listing, index) => ( // error part fixed
