...
const CredentialVerify = () => {
    ...
    const fetchAllUnverifiedListings = async () => {
        try {
            ...
            const { data } = await api.get('/api/admin/unverified-listings',{headers: {
                                ...
                                {listings.map((listing, index) => (
                                    <tr key={index} className='border-t border-gray-200 hover:bg-indigo-50/50'>
                                        ...
                                        <td className='px-4 py-3 text-center'>
                                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${listing.isCredentialSubmitted ? 'bg-yellow-100 text-yellow-600' : 'bg-red-100 text-red-600'}`}>
                                                {listing.isCredentialSubmitted ? 'Submitted' : 'Not Submitted'}
                                            </span>
                                        </td>
