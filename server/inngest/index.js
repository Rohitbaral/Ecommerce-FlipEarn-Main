...
const sendPurchaseEmail = inngest.createFunction(
  { id: 'send-purchase-email', event: "app/purchase" },
  async ({ event }) => {
    ...
    // Use updated credentials if available, otherwise original
    const credentialsToDeliver = credential.updatedCredential.length > 0 
    ...
    await sendEmail({
      ...
      <div>
      ${credentialsToDeliver.map((cred)=> `<p><strong>${cred.name}</strong> : ${cred.value}</p>`).join("")}
      </div>
...
const sendNewCredentials = inngest.createFunction(
  { id: 'send-new-credentials', event: "app/listing-deleted" },
  async ({ event }) => {
